...
class TS3_Model extends CI_Model {
...
/**
  ...
  */
	function channelInfo($cid) {
		if(!$this->runtime['selected']) { return $this->checkSelected(); }
		return $this->getData('multi', 'channelinfo cid='.$cid);
	}
 ...
 * @return	void
*/
	function __construct($timeout = 2) {
		
		if(TS3_QUERYPORT >= 1 and TS3_QUERYPORT <= 65536) {
			if($timeout >= 1) {
				$this->runtime['host'] = TS3_HOST;
				$this->runtime['queryport'] = TS3_QUERYPORT;
				$this->runtime['timeout'] = $timeout;
        if($this->getElement('success', $this->connect())) {
          $this->login(TS3_USER, TS3_PASSWORD);
          $this->selectServer(TS3_PORT);
        }
			}else{
...
/**
  *BBCODE TRANSLATOR for v2.0*
*/

	public function BBCode($string) {
		$string = nl2br($string); // new line to br
		$string = htmlspecialchars($string); // escape html chars
		
		$string = preg_replace("#\[b\](.*?)\[/b\]#si",'<b>\\1</b>',$string); // bold
		$string = str_replace('\n','<br>',$string); // new line /n
		$string = str_replace('[hr]','<hr>',$string);
		$string = str_replace('[u]','<u>',$string); // u
		$string = str_replace('[/u]','</u>',$string); // /u
		$string = preg_replace("/\[color=(\#[0-9A-F]{6}|[a-z]+)\](.*)\[\/color\]/Usi", "<span style=\"color:\\1\">\\2</span>", $string); // colors
		$string = preg_replace("/\[size=(.*)](.*)/Usi", "\\2", $string); // size
		$string = str_replace('[/size]','',$string); // /size
    // EMOJI
    $string = str_replace(':)','<i class="em em-smiley"></i>',$string);
    $string = str_replace(';)','<i class="em em-wink"></i>',$string);
    $string = str_replace(':P','<i class="em em-yum"></i>',$string);
    $string = str_replace(';P','<i class="em em-stuck_out_tongue_winking_eye"></i>',$string);
    $string = str_replace(':(','<i class="em em-worried"></i>',$string);
    $string = str_replace(';(','<i class="em em-weary"></i>',$string);
    $string = str_replace(':D','<i class="em em-joy"></i>',$string);
    $string = str_replace(';D','<i class="em em-sweat_smile"></i>',$string);
    $string = str_replace(':*','<i class="em em-kissing_face"></i>',$string);
    $string = str_replace(':O','<i class="em em-hushed"></i>',$string);
		
		return $string;
	}
	
	
	public function isAdminGroup($id) {
		
		$adminGroups = array(1, 2, 3, 4, 5, 63, 58, 70, 71, 93, 98, 100, 126, 94, 59, 60, 118, 129, 131, 130, 151, 152, 132, 153, 155, 159, 160, 162, 166, 169, 203, 210, 211, 218, 216, 214, 233, 230, 229, 247, 272, 274, 275, 276, 283, 282, 299);
	...
	public function isVip($groups) {
		if(strpos($groups, "216")) { // ROOT
     	 	return 4;
   		 } else if(strpos($groups, "131")) { //mod
			return 2;
		} else if(strpos($groups, "130")) { //admin
			return 3;
		} else if (strpos($groups, "162")) { //vip
			return 1;
		} else if (strpos($groups, "276")) { // DONATOR
      		return 5;
    	} else if (strpos($groups, "216")) {
    		return 6;
    	}
	}
  ...
  public function getUserInfoByIP($clientIP) {
    $clients = $this->clientList("-ip -client_database_id -uid -times -country -info -groups");
    $state = FALSE;
    foreach($clients['data'] as $client) {
      if($client['connection_client_ip'] == $clientIP) {
        $state = TRUE;
        return $client;
      }
    }
  ...
  public function getUserAvatar($uid) {
    $av_file = base_url().'../assets/internal/avatar_'.$this->ts3_base16(base64_decode($uid));
    $av_headers = @get_headers($av_file);
    if($av_headers[0] == 'HTTP/1.1 404 Not Found') {
  ...
  {
    if($id < 0) $id = sprintf('%u', $id & 0xffffffff);
    return $id;
  }
